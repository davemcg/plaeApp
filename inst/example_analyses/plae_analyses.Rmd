...
Let us now demonstrate how having a deep and sort of well annotated set of data can be of value in asking more specific questions, like "how do macula/fovea cones differ relative to peripheral cones."
...
Now scEiaD is based on the Ensembl gene id (like ENSG00000185527). That's not very friendly to humans so I added a bit of code to link the Ensembl ID to the gene name and plot the top 16 diff genes (padj < 0.05). 
```{r}
...
Now we see the opposite side - the top gene that is higher in peripheral vs macula. Again looks fairly consistent across all the studies.
```{r}
VlnPlot(scEiaD__subset, features = c('ENSG00000106211'))

VlnPlot(scEiaD__subset, c('ENSG00000106211'), split.by = 'regionCT', group.by='study_accession', ) + aes(color=scEiaD__subset$regionCT)
```

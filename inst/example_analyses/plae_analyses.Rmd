...
load('~/data/scEiaD_v3/scEiaD_all_seurat_v3.Rdata')
```
...
# filter down scEiaD
scEiaD_droplet__cone <- subset(scEiaD_droplet, idents = c('Macula_Cone_Homo sapiens','Peripheral_Cone_Homo sapiens'))

# create SCE object so we can use the scran test which balances covariates
sce <- as.SingleCellExperiment(scEiaD_droplet__cone)
...
symbols <- mapIds(org.Hs.eg.db, keys = row.names(diff_test), keytype = "ENSEMBL", column="SYMBOL")
sce_diff_test$`Peripheral_Cone_Homo sapiens` %>% as_tibble(rownames = 'name') %>% left_join(symbols %>% enframe()) %>% head(20)
```
